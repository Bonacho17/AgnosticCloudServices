...
export class TreeNodeFactory implements TreeEditor.NodeFactory {
    ...
    mapDataToNodes(treeData: TreeEditor.TreeData): TreeEditor.Node[] {
        ...
        if (node) {
            ...
            /* DEBUG */
            /*console.log("++++++++++ [SERVICE NAME]: " + node.jsonforms.data.children[0].serviceConfig.serviceName);
            console.log("++++++++++ [CONTAINER IMAGE]: " + node.jsonforms.data.children[0].container.containerImagePath);
            console.log("++++++++++ [PORT]: " + node.jsonforms.data.children[0].serviceConfig.port);
            console.log("++++++++++ [PROVIDER]: " + node.jsonforms.data.children[0].cloudProvider.vendor);
            console.log("++++++++++ [K8s NAMESPACE]: " + node.jsonforms.data.children[0].cloudProvider.namespace);*/
            //console.log("++++++++++ " + node.jsonforms.property);
            //console.log("++++++++++ " + node.jsonforms.index);
    ...
    hasCreatableChildren(node: TreeEditor.Node): boolean {
        return node ? DeploymentModel.childrenMapping.get(node.jsonforms.type) !== undefined : false;
    }
